    ...
    with torch.no_grad():
        for _, batch in enumerate(data_loader):
            ...
            for prediction, reference in zip(outputs.cpu().numpy(), batch['labels'].numpy()):
                ...
                valid_reference_ids = [token_id for token_id in reference if token_id != -100]
                reference_decoded = tokenizer.decode(valid_reference_ids, skip_special_tokens=True).replace(" ", "")
                for bleu_evaluator in bleu_evaluators:
                    bleu_evaluator.add_instance(prediction=prediction_decoded, references=[reference_decoded])
    model.train()
    ...
    for epoch in range(1, args.num_train_epochs+1):
        for batch in train_dataloader:
            ...
            if global_step % args.valid_steps == 0:
                ...
                bleu1, bleu2, bleu3, bleu4 = evaluate_model(model, eval_dataloader, tokenizer)
